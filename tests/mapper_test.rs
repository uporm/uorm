...
#[derive(Serialize)]
struct NameAgeArg { name: String, age: i32 }

#[derive(Serialize)]
struct UserDetailsArg { id: i64, full: bool }

#[derive(Serialize)]
struct SearchUsersArg { name: String, min_age: i32 }

#[derive(Serialize)]
struct ActiveAdultsArg { active: bool, age: i32 }

#[derive(Serialize)]
struct IdsArg { ids: Vec<i64> }

#[derive(Serialize)]
struct UpdateAgeArg { id: i64, age: i32 }

#[derive(Serialize)]
struct UpdateSelectiveArg { id: i64, name: Option<String>, age: Option<i32> }

#[derive(Serialize)]
struct MaxAgeArg { max_age: i32 }

#[derive(Serialize)]
struct UserArg { user: User }

#[derive(Serialize)]
struct UsersArg { users: Vec<User> }
...
fn init_logger() {
    INIT.call_once(|| {
        ...
        UORM.assets("tests/resources/mapper/user.xml").unwrap();
    });
}
...
async fn setup_mapper(db_name: &str) -> (Mapper, Box<dyn Connection>) {
    ...
    let mapper = UORM.mapper_by_name(db_name).unwrap();
...
async fn test_simple_select() {
    ...
    mapper.execute::<i64, _>("user.insert", &NameAgeArg {
        name: "Alice".to_string(),
        age: 20
    }).await.unwrap();

    // Test get_by_id
    let users: Vec<User> = mapper.execute("user.get_by_id", &IdArg { id: 1 }).await.unwrap();
...
async fn test_insert() {
    ...
    let affected: i64 = mapper.execute("user.insert_user", &UserArg { user: new_user }).await.unwrap();
    println!("Insert user affected: {}", affected);

    let user: Vec<User> = mapper.execute("user.get_by_id", &IdArg { id: 10 }).await.unwrap();
    ...
    let users_to_insert = vec![
        User { id: None, name: Some("Charlie".to_string()), age: Some(30), status: None, create_time: None },
        User { id: None, name: Some("David".to_string()), age: Some(35), status: None, create_time: None },
    ];
    let batch_affected: i64 = mapper.execute("user.batch_insert", &UsersArg { users: users_to_insert }).await.unwrap();
...
async fn test_conditional_select() {
    ...
    // Insert test data
    mapper.execute::<i64, _>("user.insert", &NameAgeArg { name: "Alice".to_string(), age: 20 }).await.unwrap();
    mapper.execute::<i64, _>("user.insert", &NameAgeArg { name: "Bob".to_string(), age: 25 }).await.unwrap();

    // Test get_user_details with full = true
    let details: Vec<User> = mapper.execute("user.get_user_details", &UserDetailsArg { id: 1, full: true }).await.unwrap();
    println!("User details (full=true): {:?}", details);
    assert!(details[0].status.is_some());

    // Test get_user_details with full = false
    let details_simple: Vec<User> = mapper.execute("user.get_user_details", &UserDetailsArg { id: 1, full: false }).await.unwrap();
    ...
    let searched: Vec<User> = mapper.execute("user.search_users", &SearchUsersArg {
        name: "%o%".to_string(),
        min_age: 20
    }).await.unwrap();
    ...
    let active_adults: Vec<User> = mapper.execute("user.list_active_adults", &ActiveAdultsArg {
        active: true,
        age: 18
    }).await.unwrap();
...
async fn test_foreach() {
    ...
    mapper.execute::<i64, _>("user.insert", &NameAgeArg { name: "Alice".to_string(), age: 20 }).await.unwrap();
    mapper.execute::<i64, _>("user.insert", &NameAgeArg { name: "Bob".to_string(), age: 25 }).await.unwrap();
    mapper.execute::<i64, _>("user.insert", &NameAgeArg { name: "Charlie".to_string(), age: 30 }).await.unwrap();

    // Test list_by_ids
    let ids_users: Vec<User> = mapper.execute("user.list_by_ids", &IdsArg { ids: vec![1, 3] }).await.unwrap();
...
async fn test_update_delete() {
    ...
    // Test update_age
    let update_affected: i64 = mapper.execute("user.update_age", &UpdateAgeArg { id: 1, age: 21 }).await.unwrap();
    println!("Update age affected: {}", update_affected);
    let user1: Vec<User> = mapper.execute("user.get_by_id", &IdArg { id: 1 }).await.unwrap();
    assert_eq!(user1[0].age.unwrap(), 21);

    // Test update_user_selective
    let update_selective_affected: i64 = mapper.execute("user.update_user_selective", &UpdateSelectiveArg { id: 1, name: Some("Alicia".to_string()), age: None }).await.unwrap();
    println!("Update selective affected: {}", update_selective_affected);
    let user1_updated: Vec<User> = mapper.execute("user.get_by_id", &IdArg { id: 1 }).await.unwrap();
    ...
    // Test delete_by_id
    let delete_affected: i64 = mapper.execute("user.delete_by_id", &IdArg { id: 1 }).await.unwrap();
    ...
    let delete_cond_affected: i64 = mapper.execute("user.delete_by_condition", &MaxAgeArg { max_age: 30 }).await.unwrap();
